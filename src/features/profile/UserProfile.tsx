...
async function getUserPicture(instance: IPublicClientApplication) {
  ...
  try {
    const profilePhoto = await axios.get<Blob>(
      msGraphEndpoints.userProfilePicture,
      {
        headers: {
...
export function UserProfile() {
  ...
  if (error)
    return <div>Fehler beim Laden des Benutzerprofils: {error.message}</div>;
  if (pictureError)
    return (
      <div>Fehler beim Laden des Profilbildes: {pictureError.message}</div>
    );
