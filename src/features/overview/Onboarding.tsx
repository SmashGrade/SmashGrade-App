...
async function getOnboardingFilter(): Promise<OnboardingFilterResponse> {
  const response =
    await axios.get<OnboardingFilterResponse>('/onboardingFilter');
...
export default function Onboarding({ isReadonly }: Readonly<OnboardingProps>) {
  ...
  const defaultCurriculumTypeDescription =
    firstCurriculumType?.description ?? '';

  const [currentYear, setCurrentYear] = useState(
    yearData?.length ? yearData[0].value : 0
  );

  const [selectedCurriculumType, setSelectedCurriculumType] = useState(
    defaultCurriculumTypeDescription
  );
  ...
  const curriculumTypes = onboardingData?.curriculumTypes.map(
    (curriculumType) => ({
  ...
  const availableCurriculums = onboardingData?.curriculums?.length
    ? onboardingData?.curriculums
        ...
        .filter(
          (curriculum) => curriculum.curriculumType === selectedCurriculumType
        )
        .map((curriculum) => ({
          value: curriculum.id,
  ...
  return (
    <div className={styles.overviewContainer}>
      ...
      <SelectWithTitle
        ...
        selectProps={{
          defaultValue: availableCurriculums?.length
            ? availableCurriculums[0].value
            : null,
          value: availableCurriculums?.length
            ? availableCurriculums[0].value
            : null,
