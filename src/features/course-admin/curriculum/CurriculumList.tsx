...
import {
  ...
  getCurriculums,
} from '@features/course-admin/curriculum/curriculumApi.ts';
...
export default function CurriculumList() {
  ...
  const [curriculumIdToDelete, setCurriculumIdToDelete] = useState<
    number | null
  >(null);
  ...
  const curriculumColumnDefs: ColDef<CurriculumObject>[] = [
    { field: 'focus', headerName: 'Studiengang', flex: 1 },
    {
      ...
      valueFormatter: (params) =>
        Array.isArray(params.value)
          ? params.value
              .map((manager: { name: string }) => manager.name)
              .join(', ')
          : '',
    },
    {
      ...
      headerName: 'Von',
      valueFormatter: (params) =>
        typeof params.value === 'string' ? formatDate(params.value) : '',
    },
    {
      field: 'endDate',
      headerName: 'Bis',
    ...
    },
    {
      ...
      cellRenderer: (params: { data: CurriculumObject }) => (
        <DropdownCellRenderer<CurriculumObject>
          data={params.data}
          menuItems={menuItems}
        />
      ),
