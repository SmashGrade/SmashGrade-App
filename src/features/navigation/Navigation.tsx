...
const teacherNavItems: ItemType[] = [
  {
    label: (
      <IconLink
        linkProps={{ to: MyCourseRoute.to }}
        icon={'sticky_note_2'}
        messageProps={myCourseMenu}
      />
    ),
    key: 'my-course',
  },
...
const studentNavItems: MenuProps['items'] = [
  {
    label: (
      <IconLink
        linkProps={{ to: StudentModulesRoute.to }}
        icon={'school'}
        messageProps={myCurriculumMenu}
      />
    ),
    key: 'my-curriculum',
    disabled: false,
  },
  {
    label: (
      <IconLink
        linkProps={{ to: OnboardingRoute.to }}
        icon={'rocket_launch'}
        messageProps={onboardingMenu}
      />
    ),
    key: 'onboarding',
    disabled: false,
...
const courseAdminNavItems: ItemType[] = [
  {
    label: (
      <IconLink
        linkProps={{ to: CurriculumRoute.to }}
        icon={'play_lesson'}
        messageProps={curriculumMenu}
      />
    ),
    key: 'curriculum',
  },
  ...
  },
  {
    label: (
      <IconLink
        linkProps={{ to: CourseIndexRoute.to }}
        icon={'developer_guide'}
        messageProps={courseMenu}
      />
    ),
    key: 'course',
  },
...
const defaultNavItems: ItemType[] = [
  {
    label: (
      <IconLink
        linkProps={{ to: SettingsRoute.to }}
        icon={'settings'}
        messageProps={settingsMenu}
      />
    ),
    key: 'settings',
  },
...
export default function Navigation() {
  ...
  const [current, setCurrent] = useState(
    currentPath.substring(1) || 'onboarding'
  );
  const userRoles = useUserRoles();
  const { instance } = useMsal();

  const onNavItemClick = useCallback<MenuClickEventHandler>(
    (e) => setCurrent(e.key),
    []
  );
